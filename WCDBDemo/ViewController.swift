...
class ViewController: UIViewController {
    ...
    @IBOutlet weak var integratedQueryBtn: UIButton!
    ...
    override func viewDidLoad() {
        ...
        DBManager.shared.createTable(name: "\(Sample.self)", model: Sample.self)
        
    }
    
    @IBAction func cleanTableAction(_ sender: Any) {
        if let isExist = try? DBManager.shared.db.isTableExists("\(Sample.self)"), isExist == true {
            try? DBManager.shared.db.delete(fromTable: "\(Sample.self)")
        }
        if let isExist = try? DBManager.shared.db.isTableExists("sampleTable"), isExist == true {
            try? DBManager.shared.db.delete(fromTable: "sampleTable")
        }
    }
